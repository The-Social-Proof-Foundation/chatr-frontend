    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div 
          ...
          className="font-plus-jakarta-sans fixed bottom-4 md:bottom-6 left-4 right-4 md:right-6 md:left-auto z-50 max-w-[500px] md:max-w-[400px] mx-auto md:mx-0 border border-[var(--border)] rounded-lg shadow-[0_6px_16px_rgba(0,0,0,0.15)]"
        >
          <div className="bg-[var(--background-transparent)] backdrop-blur supports-[backdrop-filter]:bg-[var(--background-transparent)] rounded-lg p-4 flex items-center justify-between gap-4">
            <Button 
              ...
              className="whitespace-nowrap bg-[var(--primary)] text-[var(--background)] hover:bg-[var(--primary)] hover:text-[var(--background)]"
            >
              {region === 'EU' ? 'Accept' : 'Okay'}
            </Button>
            <p className="text-xs text-[var(--primary)] font-plus-jakarta-sans font-medium">
              {getMessage()}
            </p>
          </div>
        </motion.div>
